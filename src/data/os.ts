...
/** Plain prompt label for the sticky HTML input bar (no ANSI). */
export function getPromptPlain(): string {
	return `${os.promptUser}@${os.osName}:~$ `;
}

/** Align command help rows: yellow command + optional args, then description. */
function helpLine(command: string, args: string, description: string, width = 42): string {
...
export function getWelcomeText(): string[] {
	return [
		...
		'',
		color.white("This is Lars Vonk's portfolio website, presented as a terminal interface."),
		...
		helpLine('blog list', ' [page <n>]', 'List blogs (newest first)'),
		helpLine('blog get', ' <id|slug|latest>', 'Show a full blog post'),
		helpLine('experience list', ' [page <n>]', 'List experience'),
		helpLine('experience get', ' <id>', 'Show one role'),
		helpLine('projects list', ' [page <n>]', 'List projects'),
		helpLine('projects get', ' <id>', 'Show one project'),
		helpLine('education list', ' [page <n>]', 'List education'),
		...
		helpLine('resume get default', '', 'Default resume (extended en)'),
		helpLine('resume get', ' <compact|extended> <en|nl>', 'Resume text + PDF'),
		helpLine('resume help', '', 'Resume options'),
		helpLine('clear', '', 'Clear the screen'),
		'',
		muted(`Tip: ${accent('blog get latest')}  ·  share: ${accent('/blogs/<slug>')}  ·  ${accent('resume get default')}`),
		'',
	];
}
